...
import project.dto.requestDto.EmailDto;
import project.dto.requestDto.LoginDto;
import project.dto.requestDto.PasswordDto;
import project.dto.requestDto.RegistrationDto;
...
public class ApiAuthController {
    ...
    public ResponseEntity<?> login(@RequestBody LoginDto loginDto){
        ...
        User userFromDB = userService.getUserByEmailAndPassword(loginDto.getEmail(), loginDto.getPassword());
    ...
    private ResponseEntity<ResultTrueDtoWithUser> getAuthUserResponseEntity(User userFromDB, boolean isModerator,
                                                                            boolean settings, Integer countNewPosts) {

        UserToLoginDto userFullInformation = new UserToLoginDto(userFromDB.getId(),
    ...
    private ResultFalseWithErrorsDto preRegistrationVerification(RegistrationDto registrationDto){
        ...
        if (!matcher.matches()){
            resultFalse.addNewError("name", "Имя указано неверно, доспустимая длина - от 2х до 16 символов");
        }
     ...
     */
    private ResultFalseWithErrorsDto preRestorePasswordVerification(PasswordDto passwordDto, User user){
        ResultFalseWithErrorsDto resultFalse = new ResultFalseWithErrorsDto();
        String captcha = captchaService.codeFromSecretCode(passwordDto.getCaptchaSecret());
        if (user == null){
            resultFalse.addNewError("code", "Ссылка для восстановления пароля устарела.\n" +
