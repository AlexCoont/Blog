...
import project.dto.InitEntityDto;
...
public class ApiGeneralController {
    ...
    public ResponseEntity<?> init(){
        ...
        InitEntityDto initEntityDto = new InitEntityDto(
                "DevPub", "Рассказы разработчиков", "+7 9155820925",
                "mail@mail.ru", "Alex Coont", "2020"
        );
    ...
    public ResponseEntity<?> getAllStatistics(){
        ...
        return ResponseEntity.ok(new StatisticsDto(postsCount, likesCount, dislikesCount, viewsCount,
                firstPublication));
    }
    ...
    public ResponseEntity<?> image(@RequestParam(value = "image", required = false) MultipartFile file){
        ...
        String generateDirs = randomAlphabetic(2).toLowerCase() + "/" + randomAlphabetic(2).toLowerCase() +
    ...
    public ResponseEntity<?> commentToPost(@RequestBody AddCommentToPostDto addCommentToPostDto){
        ...
        if (addCommentToPostDto.getText().length() < 6){
            ...
            resultFalseWithErrorsDto
                    .addNewError("text", "Длина комментария должна быть равна 6 или более символам");
