...
import project.dto.requestDto.ProfileDto;
import project.dto.responseDto.ResultFalseWithErrorsDto;
import project.dto.responseDto.ResultTrueDto;
...
public class ApiProfileController {
    ...
    private ResultFalseWithErrorsDto editWithPhotoVerification(MultipartFile file, ProfileDto dto, User user){
        ResultFalseWithErrorsDto errorsDto = verificationWithoutPhoto(dto, user);
    ...
    private ResultFalseWithErrorsDto verificationWithoutPhoto(ProfileDto dto, User user){
        ...
        if (!matcher.matches()){
            errorsDto.addNewError("name", "Имя указано неверно, доспустимая длина - от 2х до 16 символов");
        }
