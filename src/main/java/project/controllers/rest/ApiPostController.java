...
import project.dto.requestDto.LikeOrDislikeDto;
...
public class ApiPostController {
    ...
    public ResponseEntity<?> getAllPosts(@RequestParam Integer offset,
                                         @RequestParam Integer limit, @RequestParam String mode)
    {
    ...
    @GetMapping("/byTag")
    public ResponseEntity<?> getPostsByTag(@RequestParam String tag, @RequestParam Integer offset,
    ...
    @GetMapping("/search")
    public ResponseEntity<?> getPostsBySearchQuery(@RequestParam Integer offset,
                                                   @RequestParam Integer limit,
                                                   @RequestParam(required = false) String query)
    {
        ...
        return ResponseEntity.ok(new PostListDto(countPostsBySearchQuery, postDtoList, offset, limit, query));
    }

    @GetMapping("/byDate")
    public ResponseEntity<?> getPostsByDate(@RequestParam String date, @RequestParam Integer offset,
                                            @RequestParam Integer limit)
    {
    ...
    @GetMapping("/my")
    public ResponseEntity<?> getMyPosts(@RequestParam Integer offset, @RequestParam Integer limit,
    ...
    @PutMapping("/{id}")
    public ResponseEntity<?> editPostById(@PathVariable Integer id, @RequestBody AddPostDto addPostDto){
        Integer userId = authService.getUserIdOnSessionId();
        authService.checkAuth(userId);
        ResultFalseWithErrorsDto errorsDto = checkPostsTitleAndText(addPostDto);
        if (errorsDto.getErrors().size() > 0){
    ...
    public ResponseEntity<?> postsToModeration(@RequestParam Integer offset, @RequestParam Integer limit,
                                               @RequestParam String status){
        Integer userId = authService.getUserIdOnSessionId();
        authService.checkAuth(userId);
        User userFromDB = userService.getUserById(userId);
