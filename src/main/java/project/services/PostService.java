...
import project.dto.requestDto.AddPostDto;
import project.dto.requestDto.ModerationPostDto;
...
public class PostService {
    ...
    public List<PostDto> getAllPosts(Integer offset, Integer limit, String mode){
        ...
        switch (mode){
            case "recent":
                return getPostsDtos(postRepository.findAllByModerationStatusAndIsActiveOrderByTimeDesc
                        (ModerationStatus.ACCEPTED, (byte) 1, pageable));
            case "early":
                return getPostsDtos(postRepository.findAllByModerationStatusAndIsActiveOrderByTimeAsc
                    (ModerationStatus.ACCEPTED, (byte) 1, pageable));
            case "popular":
    ...
    public OnePostDto getPostById(Integer id, User user)
    {
        ...
        Post post = postRepository.getByIdAndModerationStatusAndIsActive(id, ModerationStatus.ACCEPTED, (byte)1);
    ...
    public List<PostDto> getPostsBySearchQuery(String query, Integer limit, Integer offset)
    {
        ...
        Pageable pageable = PageRequest.of(offset/limit, limit);
        return getPostsDtos(
                postRepository.findAllByTitleContainingAndIsActiveAndModerationStatusAndTimeIsBeforeOrderByTimeDesc
    ...
    public Integer getCountPostsBySearchQuery(String query){
        LocalDateTime time = LocalDateTime.now();
        return postRepository.countAllByTitleContainingAndIsActiveAndModerationStatusAndTimeIsBeforeOrderByTimeDesc
    ...
    /**
     * расчет количества всех постов определенного пользователя в зависимости от активности и статуса модерации
     */
    ...
    /**
     * получение постов для модерации, или постов, которые утвердил или отклонил определенный модератор
     */
    @Transactional
    public PostListDto getPostsToModeration(Integer offset, Integer limit, String status, Integer moderatorId){
        Pageable pageable = PageRequest.of(offset/limit, limit);
        List<PostDto> postDtoList = new ArrayList<>();
        Integer count;
        switch (status){
            case "new":
                postDtoList.addAll(getPostsDtos(postRepository
                        .findAllByModerationStatusAndIsActiveOrderByTimeDesc(ModerationStatus.NEW,
                                (byte) 1, pageable)));
                count = getCountPostsToModeration();
                return new PostListDto(count, postDtoList, offset, limit, status);
            case "accepted":
                postDtoList.addAll(getPostsDtos(postRepository
                        .findAllByIsActiveAndModerationStatusAndAndModeratorId((byte) 1, ModerationStatus.ACCEPTED,
                                moderatorId, pageable)));
                count = postRepository.countAllByModerationStatusAndAndIsActiveAndAndModeratorId
                        (ModerationStatus.ACCEPTED, (byte) 1, moderatorId);
                return new PostListDto(count, postDtoList, offset, limit, status);
            case "declined":
                postDtoList.addAll(getPostsDtos(postRepository.findAllByIsActiveAndModerationStatusAndAndModeratorId
    ...
    /**
     * --------------------------------------------------------
     * приватные методы для различных превращений и не только..
     */
    private List<PostDto> getPostsDtos(List<Post> postList){
        return postList.stream().map(post -> new PostDto(
                post.getId(), post.getTime(), new UserToPostDto(post.getUserId().getId(), post.getUserId().getName()),
                post.getTitle(), post.getText().substring(0, numberForAnnounce),
                (int) (post.getPostVoteList().stream().filter(postVote -> postVote.getValue() == 1).count()),
                (int) (post.getPostVoteList().stream().filter(postVote -> postVote.getValue() == -1).count()),
                post.getPostCommentList().size(), post.getViewCount()
        )).collect(toList());
    }

    private List<String> getPostTagList(Post post){
        return post.getTagList().stream().map(Tag::getName)
                .collect(toList());
    }

    private List<CommentToPostDto> getCommentToPostDto(List<PostComment> postCommentList){
        return postCommentList.stream().map(postComment ->
                new CommentToPostDto(postComment.getId(), postComment.getTime(), postComment.getText(),
                        new UserToPostCommentDto(postComment.getUserId().getId(),
                                postComment.getUserId().getName(), postComment.getUserId().getPhoto())))
                                    .collect(toList());
    }
    ...
    private OnePostDto getPostByIdDto(Post post)
    {
        ...
        return new OnePostDto(post.getId(), post.getTime(),
                new UserToPostDto(post.getUserId().getId(), post.getUserId().getName()),
                post.getTitle(), post.getText().substring(0, numberForAnnounce),
                (int) (post.getPostVoteList().stream().filter(postVote -> postVote.getValue() == 1).count()),
                (int) (post.getPostVoteList().stream().filter(postVote -> postVote.getValue() == -1).count()),
                post.getPostCommentList().size(), post.getViewCount(), post.getText(), commentToPostDtoList,
                tagList
                );
    }
